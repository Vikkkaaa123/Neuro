...
namespace MO_32_2_Pereuznik_esc.NeuroNet
{
    abstract class Layer //модификаторы protected стоят для внутрииерархического использования членов
    {
        ...
        protected const double learningrate = 0.0541; //скорость обучения 0.085
        protected const double momentum = 0.0522d; //момент итерации 0.085  0.05
        ...
        protected Layer(int non, int nopn, NeuronType nt, string nm_Layer) //nm_Layer используется для идентификации слоя
        {
            ...
            if (File.Exists(pathFileWeights)) //опрделяет, существует ли pathFileWeights
            ...
            //цикл формиррования нейрона слоя
            for (int i = 0; i < non; i++)
            {
        ...
        public double[,] WeightsInitialize(MemoryMode mm, string path)
        {
            ...
            switch (mm)
            {
                ...
                case MemoryMode.INIT:
                    // Инициализация синаптических весов по методу He для LeakyReLU
                    // (улучшенная версия Xavier инициализации для LeakyReLU)
                    double alpha = 0.01; // коэффициент LeakyReLU
                    int fanIn = numofprevneurons; // количество входов
                    ...
                    for (int i = 0; i < numofneurons; i++)
                    {
                        for (int j = 0; j < numofprevneurons + 1; j++)
                        {
                            ...
                            {
                                // Веса инициализируем равномерным распределением в пределах [-limit, limit]
